...
The notebook kernel’s current working directory must be the project folder so that relative paths like `'data_catalog.parquet'` resolve. Run this in the **first cell**:
...
df = duckdb.query("SELECT * FROM read_parquet('data_catalog.parquet') LIMIT 10").df()

# Option B: absolute path (works regardless of kernel cwd)
path = os.path.join(os.getcwd(), 'data_catalog.parquet')
