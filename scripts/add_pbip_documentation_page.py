...
def build_field_guide_page(page_dir: Path) -> None:
    ...
    slicer_h = 100
    slicer2_y = slicer_y + slicer_h + 8
    table_y = slicer2_y + slicer_h + 16
    ...
    visuals = [
        ...
        textbox(
            ...
            "Understand each column: layer (Catalog/Dictionary/Context), interoperability role, and Excel source",
        ...
        textbox(
            vid(), margin, slicer2_y, content_w, slicer_h, 5,
            "Layers: Catalog = WHAT (USCDI) | Dictionary = HOW (FHIR/US Core) | Terminology/Crosswalk = WHICH codes | Message context = WHERE (ADT/CCDA) | Sources = feed coverage. Authoring: chi-steward-workbook.xlsx → import script.",
            size="11pt", color=TEXT_BLACK,
        ),
        guide_detail_table(vid(), margin, table_y, content_w, table_h, 6),
        shape_rect(vid(), 0, h - footer_h, w, footer_h, 7, PRIMARY_YELLOW),
        textbox(
            vid(), margin, h - footer_h + 10, content_w, 36, 8,
            "Generated by scripts/generate_pbip_model_guide.py | docs/faq.md | Not steward-authored clinical data",
