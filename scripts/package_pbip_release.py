"""Package PBIP, parquet, and Excel workbooks for deployment on another Windows PC.

The semantic model uses absolute paths to:
  C:\\AI\\chiddc\\ddc-*.parquet

Extract the zip to that exact folder on the target machine, then open:
...
echo Start on Guide · Start here, then use Guide · Walkthrough if you want a guided tour.
pause
"""


def package_readme() -> str:
    return f"""CHI Data Dictionary Catalog - Power BI release package
============================================================
...
Quick start: double-click OPEN-CATALOG.bat (or open the PBIP path below).
Read WALKTHROUGH.txt for the same steps as the in-report Guide · Walkthrough tab.
...
Default tab: Guide · Start here. Then Home -> Refresh from the REPORT view.
...
If you cannot use {REPO_PARQUET} on the target PC:
...
PBIP is the maintained report format in git.
"""
...
def package(out_zip: Path, *, native_parquet: bool = False) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        from pbip_walkthrough_content import render_walkthrough_txt

        with zipfile.ZipFile(out_zip, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            zf.writestr(f"{RELEASE_ROOT_NAME}/PACKAGE-README.txt", package_readme())
            zf.writestr(f"{RELEASE_ROOT_NAME}/WALKTHROUGH.txt", render_walkthrough_txt())
            zf.writestr(f"{RELEASE_ROOT_NAME}/OPEN-CATALOG.bat", open_catalog_bat())

            for name in PARQUET_FILES:
                src = REPO_ROOT / name
                if native_parquet:
                    zf.write(src, f"{RELEASE_ROOT_NAME}/{name}")
                else:
                    compat = tmp_path / name
                    write_pbi_compatible_parquet(src, compat)
                    zf.write(compat, f"{RELEASE_ROOT_NAME}/{name}")

            for rel in EXCEL_FILES:
                src = REPO_ROOT / rel
                zf.write(src, f"{RELEASE_ROOT_NAME}/{Path(rel).as_posix()}")

            for src in PBIP_DIR.rglob("*"):
                if src.is_dir() or should_skip(src.relative_to(PBIP_DIR)):
                    continue
                rel = src.relative_to(REPO_ROOT)
                zf.write(src, f"{RELEASE_ROOT_NAME}/{rel.as_posix()}")

    print(f"Wrote {out_zip}")
    print(f"On target PC: extract to C:\\AI\\ then open workbooks\\pbip\\{PBIP_FILE}")
