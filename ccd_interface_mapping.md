...
- **Same L2 vocabulary**: INV columns (e.g. `fn`, `ln`, `psa1`, `efdt`, `eid`, `et`, `pd`, `inpid`) align with **data/l2_to_semantic_id_mapping.csv** and CMT ADT. That lets you:
  - Extend **ccda_catalog.parquet** with more CCD section/XPath → semantic_id rows.
...
- **Reference only**: This is reference material. To drive the app or exports, add chosen rows into **ccda_catalog.parquet** (and optionally the master catalog) via script or manual curation.
...
You can use this mapping to add rows to **ccda_catalog.parquet** (CCD XPath → semantic_id or INV COLUMN → semantic_id) so the Streamlit app shows CCD mappings alongside ADT for the same elements.
